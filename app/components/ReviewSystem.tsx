...
export default function ReviewSystem({ companyId }: ReviewSystemProps) {
    ...
    return (
        <section className="space-y-6">
            ...
            {/* Average */}
            <div className="flex flex-col sm:flex-row items-center gap-4 p-5 bg-amber-50 rounded-2xl border border-amber-100">
                <div className="flex flex-col items-center">
                    <span className="text-5xl font-bold text-amber-500 leading-none">
            ...
            {userId && !alreadyReviewed && !success && (
                <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl p-5 space-y-4">
                    <h3 className="font-semibold text-gray-800">Laisser un avis</h3>
                    <div>
                        <label className="block text-sm text-gray-600 mb-1">Note *</label>
                        <StarRating value={rating} onChange={setRating} size="lg" />
                    </div>
                    <div>
                        <label className="block text-sm text-gray-600 mb-1">Commentaire (optionnel)</label>
                        <textarea
                            value={comment}
            ...
            {reviews.length === 0 ? (
                ...
                <ul className="space-y-3">
                    {reviews.map((r) => (
                        <li key={r.id} className="bg-white border border-gray-100 rounded-2xl p-4 space-y-1">
